...
export default function InvoiceFormDirect({ onSuccess, onCancel, editQuoteId }: Props) {
  ...
  const updateMutation = trpc.commercial.invoices.createWithLines.useMutation({
    onSuccess: () => {
  ...
  return (
    <Card className="p-6 bg-white border border-slate-200 space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">{isEdit ? `Nouvelle facture` : "Nouvelle facture"}</h3>
      ...
      <div className="flex justify-end gap-2 pt-4 border-t">
        ...
        <Button onClick={handleSubmit} disabled={createMutation.isPending || updateMutation.isPending}>
          {(createMutation.isPending || updateMutation.isPending) ? "Enregistrement…" : (isEdit ? "Mettre à jour" : "Créer la facture")}
        </Button>
      </div>
    </Card>
  );
